...
export const SLIDES: Slide[] = [
  ...
  },
  {
    ...
    bullets: [
      "OPTION:A. Title Tag 후보|웹사이트의 각 페이지에 붙이는 '이름표'|음식물 처리의 한 손 시대 개막 (비전형)|가정용 음식물처리기 공식 온라인몰 (명확형)|원핸드그립 음식물처리기, 블루벤트 ID (제품형)|한 손으로 끝내는 음식물처리기 공식 온라인몰 (혼합형)",
      "OPTION:B. H1 Tag 후보|페이지의 핵심 주제를 정의하는 대제목|한 손으로 끝내는 음식물처리기, 블루벤트 (USP)|한 손으로 넣고 AI로 처리하는 음식물처리기 (AI 강조)|블루벤트, AI 음식물처리기의 새로운 기준 (리더십)",
      "OPTION:C. FAQ 5개 선정|사용자 의도를 반영한 텍스트 블록 (카테고리 노출 여부)|필터 교체 주기 (필터 사용 및 관리)|닭뼈/생선뼈 처리 여부 (제품 사용법)|추가 투입 가능성 (제품 사용법)|작동 소음 정도 (제품 문의)|처리 완료 시간 (제품 문의)",
